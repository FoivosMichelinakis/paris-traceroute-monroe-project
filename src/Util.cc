...
Util::getRoute (const char* dest) {
  ...
  char buff[20];
	int sock;
	struct sockaddr_in addr, name; //http://www.gta.ufrj.br/ensino/eel878/sockets/sockaddr_inman.html  http://www.retran.com/beej/sockaddr_inman.html
	int len;
	char *dst_addr;
  // Ouvre un tube nomm�
	...
	//std::cout << strdup(inet_ntoa(name.sin_addr)) << std::endl;
