...
void Output::text(FILE* out, Tracert *results, Options* opts) {
	fprintf(out,
			"traceroute [(%s:%d) -> (%s:%d)], protocol %s, algo %s, duration %d s\n",
			opts->src_addr, opts->src_port, opts->dst_addr, opts->dst_port,
	...
	for (int i = results->getMinTTL(); i <= results->getMaxTTL(); i++) {
		...
		if (exh_output)
			char_count += fprintf(out, " P(%d, %d)", results->getNbrReplies(i),
					results->getNbrProbes(i));
		...
		for (int j = 0; j < results->getNbrProbes(i); j++) {
			...
			if (!exh_output) {
				...
				if (last_tprobe == NULL
						|| (tprobe->host_address_raw
								!= last_tprobe->host_address_raw)) {
				...
				if (show_interface) {
					...
					if (/*results->getLoadBalancingType(i) == 1
					 &&*/results->getNbrInterfaces(i) > 1) {
						...
						if (!exh_old
								|| results->getLoadBalancingType(i, j) == 1) {
							fprintf(out, ":%d", tprobe->flow_identifier);

							// Find all the probes that returned the same interface
							for (int k = j + 1; k < results->getNbrProbes(i);
									k++) {
								const TimedProbe *tprobe2 = results->getHopInfo(
										i, k);
								if (tprobe2->arrival_time != 0
										&& tprobe2->host_address_raw
												== tprobe->host_address_raw
										// XXX ignore probes sent to classify load balancers
										&& tprobe2->flow_identifier
												!= tprobe->flow_identifier) {
									fprintf(out, ",%d",
											tprobe2->flow_identifier);
								}
							}
			...
			if (show_interface) {
				...
				if (exh_output) {
					...
					fprintf(out, "  %04.03f/%04.03f/%04.03f/%04.03f ms ", min,
							avg, max, stdev);

				} else {
					fprintf(out, "  %04.03f ms ",
							((float) tprobe->getRTT()) / 1000.0);
				} // exh output
					...
					for (int k = j + 1; k < results->getNbrProbes(i); k++) {
						...
						if (tprobe2->arrival_time != 0
							...
							for (int k2 = j; k2 < k; k2++) {
								const TimedProbe *tprobe3 = results->getHopInfo(
										i, k2);
								if (tprobe3->arrival_time != 0
										&& tprobe3->host_address_raw
												== tprobe->host_address_raw
										&& tprobe2->fabien_ttl
												== tprobe3->fabien_ttl)
									new_ttl = false;
...
//         last_tprobe = tprobe;
		...
		for (int j = 0; j < opts->max_try; j++) {
			...
			if (tprobe != NULL && tprobe->mpls_stack != NULL
					&& last_tprobe != NULL) {
				if (MPLSHeader::compareStacks(tprobe->mpls_stack,
						tprobe->nbrLabels, last_tprobe->mpls_stack,
						last_tprobe->nbrLabels) != 0) {
					fprintf(stdout, ", ");
					show_stack = true;
				}
			...
			if (show_stack) {
				fprintf(stdout, "%d TTL=%d", tprobe->mpls_stack[0],
						tprobe->mpls_ttl);
...
void Output::raw(FILE* out, Tracert *results, Options* opts) {
